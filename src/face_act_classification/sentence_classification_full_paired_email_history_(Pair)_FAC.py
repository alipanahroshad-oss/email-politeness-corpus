print("past same emails Req Plus Rep")

import os, re, ast, json, math, random, textwrap
...
CFG = {
    ...
    "SEED_COL":     "seed",
    "PAIR_COL":     "pair_idx",
    "filter_is_request": None,      # 1=requests only; 0=replies only; None=both
...
def sigmoid_stable(x: np.ndarray) -> np.ndarray:
    ...
    x = np.clip(x, -50.0, 50.0)        # prevents exp overflow
...
def build_target_and_context_req_plus_rep(df: pd.DataFrame, CFG) -> pd.DataFrame:
    ...
    For each (seed, pair_idx):
      - if is_request==1 (target is request): context = past request sentences (same pair), ordered by sentence_idx
      - if is_request==0 (target is reply):   context = full request (all sentences) + past reply sentences (same pair)
    """
...
def tokenize_pairs(target_list: List[str], context_list: List[str]):
    return tok(
        ...
        context_list,
        padding=True,
        truncation="only_second",   # trim context only
...
def debug_print_samples(df_ctx: pd.DataFrame, ds: ReqPlusRepTargetFirstDS, CFG, n=3):
    ...
    for i in chosen[:n]:
        ...
        print("CONTEXT:", textwrap.shorten(row['context'], width=160, placeholder='…'))  # spaces only
...
trainer = ASLTrainer(
    ...
    eval_dataset=val_ds,
    tokenizer=tok,
    compute_metrics=compute_metrics_training,  # fixed τ=0.5 for epoch table
...
def metrics_from_bin(gold: np.ndarray, pred: np.ndarray) -> Dict[str,float]:
    hamming = np.not_equal(pred, gold).mean()  # robust (no XOR on floats)
    return {
...
print(f"[VAL][Global τ@F1] τ={tau_f1_forced:.2f}  metrics=")
...
test_main, test_bin = eval_with_tau(test_logits, test_labels, tau_f1_forced)       # forced τ=0.60
...
_ = metrics_subset(test_labels, test_bin,  mask_req, "REQUEST")
_ = metrics_subset(test_labels, test_bin,  mask_rep, "REPLY")
...
if CFG["save_test_predictions"]:
    ...
    print("[TEST] Wrote per-reply micro-F1 →", out_rep_csv)
# i used the results of this code to the paper