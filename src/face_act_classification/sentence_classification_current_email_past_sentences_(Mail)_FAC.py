print("past same email request Or rep BERT")
...
def tokenize_pairs(target_list: List[str], context_list: List[str]):
    return tok(
        ...
        context_list,
        padding=True,
        truncation="only_second",   # trim context only; keep target whole
...
trainer = ASLTrainer(
    ...
    eval_dataset=val_ds,
    tokenizer=tok,
    compute_metrics=compute_metrics_training,  # τ=0.50 for epoch table comparability
...
tau_f1 = float(CFG["FORCE_TAU_F1"])  # << force for comparability

print(f"[VAL][Global τ@F1] τ={tau_f1:.2f}  metrics=")
...
is_req_flags = df_test_ctx[CFG["ISREQ_COL"]].values.astype(int)  # 1=request, 0=reply
mask_req = (is_req_flags == 1)
mask_rep = (is_req_flags == 0)

_ = metrics_from_mask(test_labels, test_bin, mask_req, "REQUEST")
_ = metrics_from_mask(test_labels, test_bin, mask_rep, "REPLY")
